...
max_nf_delay = 10.0
wait_time = 50
network = sfcsim.cernnet2_sfc_dynamic(num_sfc = 1000)
...
class NFVEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, num_sfc=100):
        super().__init__()
        self._dep_fin = False#看了看后面的代码，没派上用处
        ...
        self._episode_ended = False  #部署完一条服务功能链
    ...
    def _reset(self, full_reset=False):
        if self._sfc_index == (self.network.sfcs.get_number()) or full_reset == True:#为什么不用dep_fin来替代？
