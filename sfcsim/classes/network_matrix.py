...
*************************************************************************************
    ...
    network_matrix类,矩阵形式的网络存储结构，可以直接转换为network类（）只是存储数据，里面并没有node的引用，需要更新时由network类
    generate更新
    属性值：
        node_list            节点id数组
        edges                链路连接情况矩阵
...
************************************************************************************* 
    ...
    def __init__(self,node_list=[],edge=[],edge_atts={},node_atts={}):
        ...
        self.node_atts=copy.deepcopy(node_atts)#注意，里面的是剩余资源！
    ...
    def generate(self,network1):  #从network类转换成network_matrix类
        ...
        for node in network1.get_nodes():
            ...
            for attr in node.get_remain_resource():#注意，里面是
