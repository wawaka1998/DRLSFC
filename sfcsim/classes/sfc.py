...
class sfc():
    ...
    def __init__(self, uuid, in_node, out_node, nfs=[], bandwidth=0, delay=0, start_time = 0 ,duration=0, profit=0, vnf_types=[]):
        self.id = uuid
        self.atts = {}
        ...
        self.atts['start_time'] = start_time
...
class sfcs():
    ...
    def show(self):
        ...
        print('%-6s %-8s %-7s %-8s %-5s %-8s %-6s %-32s %-s' % (
        'number', 'id', 'in_node', 'out_node', 'delay', 'duration', 'profit', 'bandwidths', 'nfs'))
        i = 1
        for sfc in self.sfcs:
            print('%-6d %-8s %-7s %-8s %-5d %-8d %-6d %-32s %-s' % (
            i, sfc.get_id(), sfc.get_in_node(), sfc.get_out_node(), \
